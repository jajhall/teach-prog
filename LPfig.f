      ...
      axis_mn_x = -2
      axis_mx_x = 6
      print*, 'Enter min and max for x1: dl = mu * 8.5'
     ...
     &     (axis_mx_x-axis_mn_x)
      yscl = -xscl
      y_paper_ht = -yscl*(axis_mx_y-axis_mn_y)+
 ...
 9000 format('#FIG 3.2'/'Portrait'/'Center'/'Metric'/
     &     'A4      '/'100.00'/'Single'/'-2'/'1200 2'/)
      ...
      if (sans_serif) then
         write(wr_cn, 9005)fig_mx_x, y0+300, 'x', backslash
         write(wr_cn, 9007)fig_mx_x+50, y0+350, '1', backslash
      else
         write(wr_cn, 9010)fig_mx_x, y0+300, 'x', backslash
         write(wr_cn, 9012)fig_mx_x+50, y0+350, '1', backslash
      endif
!      write(wr_cn, 9015)fig_mx_x, y0, '$x_1$', backslash
      write(wr_cn, 9000)x0, fig_mx_y, x0, fig_mn_y
      if (sans_serif) then
         write(wr_cn, 9005)x0-250, fig_mn_y, 'x', backslash
         write(wr_cn, 9007)x0-200, fig_mn_y+50, '2', backslash
      else
         write(wr_cn, 9010)x0-250, fig_mn_y, 'x', backslash
         write(wr_cn, 9012)x0-200, fig_mn_y+50, '2', backslash
      endif
!      write(wr_cn, 9015)x0-250, fig_mn_y, '$x_2$', backslash
      return
 9000 format('2 1 0 2 0 7 50 0 -1 0.000 0 0 -1 1 0 2'/
     &     '1 1 2.00 120.00 240.00'/4(2x, i6))
 9005 format('4 1 0 25 0 4 32 0.0000 0 150 210',
     &     2(2x, i8), 2(1x, a1), '001')
 9007 format('4 1 0 25 0 4 18 0.0000 0 150 270',
     &     2(2x, i8), 2(1x, a1), '001')
 9010 format('4 1 0 50 0 3 32 0.0000 0 225 210',
     &     2(2x, i8), 2(1x, a1), '001')
 9012 format('4 0 0 50 0 1 18 0.0000 0 225 210',
     &     2(2x, i8), 2(1x, a1), '001')
 9015 format('4 0 0 25 -1 16 12 0.0000 6 165 510',
     &     2(2x, i8), 1x, a5, 1x, a1, '001')
      end
...
c
 ...
 15   continue
      if (x_axis) then
         ...
         if (sans_serif) then
            write(wr_cn, 9005)
     &           fig_x_v, fig_dy_v, ch6_v(f_ch:6), backslash
         else
            write(wr_cn, 9010)
     &           fig_x_v, fig_dy_v, ch6_v(f_ch:6), backslash
         endif
         ...
         if (sans_serif) then
            write(wr_cn, 9005)
     &           fig_dx_v, fig_dy_v, ch6_v(f_ch:6), backslash
         else
            write(wr_cn, 9010)
     &           fig_dx_v, fig_dy_v, ch6_v(f_ch:6), backslash
         endif
 ...
 9000 format('2 1 0 1 0 7 25 0 -1 0.000 0 0 -1 0 0 2'/
     &     4(2x, i8))
 9005 format('4 1 0 25 0 4 18 0.0000 0 150 270', 2(2x, i8), 1x, a, a1, 
     &     '001')
